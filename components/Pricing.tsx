    ...
    <section id="pricing" className="py-24 sm:py-32">
      <div className="max-w-6xl mx-auto px-6">
        ...
        >
          ...
          <h2 className="text-2xl sm:text-3xl md:text-[2.5rem] font-bold tracking-tight text-white mb-3">
        ...
        <div className="grid md:grid-cols-3 gap-5 items-start">
          {plans.map((plan, i) => (
            <motion.div
              ...
              className={`rounded-xl p-6 sm:p-7 flex flex-col relative overflow-hidden ${
                plan.highlight
                  ? "bg-gradient-to-b from-cyan-500/[0.06] to-transparent border border-cyan-500/20"
                  : "card"
              }`}
            >
              {plan.highlight && (
                <span className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-cyan-400 to-transparent" />
              )}
              {plan.highlight && (
                <span className="inline-block text-[10px] font-semibold uppercase tracking-wider text-cyan-400 bg-cyan-500/10 px-2.5 py-0.5 rounded-full mb-4 self-start">
                  Popular
                </span>
              )}
              ...
              <a
                ...
                className={`block w-full py-2.5 rounded-lg text-center text-sm font-medium transition-all duration-200 ${
                  plan.highlight
                    ? "btn-primary"
                    : "btn-secondary"
                }`}
